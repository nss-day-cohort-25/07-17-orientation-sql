...
Queries examples:
-- Get all superheroes and their powers who have pysical powers
SELECT s.name as "hero", power.name as "power" ( use group_concat(power.name) for some magic!)
FROM Superhero s
JOIN SuperheroPower sp on s.superhero_Id = sp.Superhero_Id
JOIN POWER on Power.Power_Id = sp.Power_Id
JOIN PowerType on Power.PowerType_Id = PowerType.PowerType_Id
WHERE PowerType.name = "Physical"
